/**
 * Banri Chenguang MCP Server - Online (Streamable HTTP)
 */
...
import { BanriChenguangOAuthProvider } from './oauth/provider.js';
...
app.get('/info', (_req, res) => {
  res.status(200).json({
    name: 'Banri Chenguang MCP Server',
    status: 'ok',
...
function createMcpServer(auth: McpAuthInfo) {
  const server = new McpServer(
    { name: 'banri-chenguang-mcp-online', version },
    { capabilities: { tools: {} } }
  );
...
const httpServer = app.listen(PORT, HOST, () => {
  console.log(`Banri Chenguang MCP Server (Streamable HTTP + OAuth 2.1) running on ${HOST}:${PORT} at /mcp`);
});
