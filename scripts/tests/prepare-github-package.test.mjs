...
test('prepare-github-package rewrites core metadata for GitHub Packages', async () => {
  ...
  assert.equal(manifest.name, '@hhszzzz/banri-chenguang-core');
  ...
  assert.deepEqual(manifest.repository, {
    type: 'git',
    url: 'https://github.com/hhszzzz/banri-chenguang.git',
  });
});

test('prepare-github-package copies the package-local LICENSE when available', async () => {
  const tempRoot = await mkdtemp(path.join(tmpdir(), 'banri-chenguang-license-check-'));
...
test('prepare-github-package rewrites workspace dependencies for GitHub Packages', async () => {
  ...
  assert.equal(manifest.name, '@hhszzzz/banri-chenguang-mcp');
  assert.equal(manifest.dependencies['@hhszzzz/banri-chenguang-core'], sourceCoreManifest.version);
  assert.equal(manifest.dependencies['banri-chenguang-core'], undefined);
  assert.equal(manifest.publishConfig.registry, 'https://npm.pkg.github.com');
  assert.deepEqual(manifest.repository, {
    type: 'git',
    url: 'https://github.com/hhszzzz/banri-chenguang.git',
