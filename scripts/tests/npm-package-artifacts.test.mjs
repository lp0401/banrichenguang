...
test('npm package tarballs should rewrite workspace dependencies before publish', async () => {
  ...
  const coreTarball = path.join(TMP_DIR, `banri-chenguang-core-${coreSourceManifest.version}.tgz`);
  const mcpTarball = path.join(TMP_DIR, `banri-chenguang-mcp-${mcpSourceManifest.version}.tgz`);

  packWorkspacePackage('banri-chenguang-core', coreTarball);
  packWorkspacePackage('banri-chenguang-mcp', mcpTarball);
  ...
  assert.equal(
    mcpPackedManifest.dependencies['banri-chenguang-core'],
    coreSourceManifest.version,
    'banri-chenguang-mcp should depend on the published banri-chenguang-core version in the packed artifact',
  );
});
