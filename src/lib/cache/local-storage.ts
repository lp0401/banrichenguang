...
const CACHE_SCOPE_PREFIXES: Record<Exclude<LocalCacheScope, 'default_bazi_chart'>, string> = {
  data_sources: 'banri-chenguang.data_sources.',
  knowledge_bases: 'banri-chenguang.knowledge_bases.',
};

const CACHE_SCOPE_KEYS: Record<'default_bazi_chart', string[]> = {
  default_bazi_chart: ['banri-chenguang.pref.defaultBaziChartId', 'mingai.pref.defaultBaziChartId', 'defaultBaziChartId'],
};
...
export function invalidateLocalCaches(scopes: LocalCacheScope[]) {
  ...
  for (const scope of uniqueScopes) {
    ...
    removeLocalCacheByPrefix(CACHE_SCOPE_PREFIXES[scope].replace(/^banri-chenguang/u, 'taibu'));
    removeLocalCacheByPrefix(CACHE_SCOPE_PREFIXES[scope].replace(/^banri-chenguang/u, 'mingai'));
  }
}
