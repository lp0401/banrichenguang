...
async function ensureSystemAdminUserRecord(adminUserId: string): Promise<void> {
    const adminKey = getSupabaseAuthAdminKey();
    if (!adminKey) {
        return;
    }

    const client = createClient(getSupabaseUrl(), adminKey, {
    ...
    if (error) {
        console.error('[supabase-server] Failed to ensure system admin user row:', error);
    }
}
...
async function signInSystemAdmin(): Promise<Session | null> {
    ...
    // system-admin 会话常用于绕过 RLS 执行管理操作；
    // 如果 public.users 中没有对应行或 is_admin 不为 true，
    // 依赖 is_admin 的 RLS policy 会返回空结果而非错误，导致静默失败。
    await ensureSystemAdminUserRecord(data.session.user.id);
