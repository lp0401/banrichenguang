...
function isPrivilegedAuthError(error: unknown): boolean {
...
async function callCheckinRpc(userId: string): Promise<{ data?: CheckinRpcResult; error?: unknown }> {
    const systemClient = getSystemAdminClient();
    const systemResult = await systemClient.rpc('perform_daily_checkin_as_service', {
        p_user_id: userId,
    });

    if (!systemResult.error) {
        return { data: (systemResult.data || {}) as CheckinRpcResult };
    }

    console.warn('[checkin] system-admin RPC failed:', systemResult.error);

    if (!isPrivilegedAuthError(systemResult.error)) {
    ...
    const authResult = await authAdminClient.rpc('perform_daily_checkin_as_service', {
        p_user_id: userId,
    });

    if (!authResult.error) {
        return { data: (authResult.data || {}) as CheckinRpcResult };
    }

    console.warn('[checkin] auth-admin RPC failed:', authResult.error);
...
export async function performCheckin(
    ...
    const { data, error } = await callCheckinRpc(userId);
    ...
    const result = (data || {}) as CheckinRpcResult;
