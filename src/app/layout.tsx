...
(() => {
  ...
  // Migrate legacy taibu theme keys to banri-chenguang
  const migrateThemeKey = (newKey, oldKey) => {
    const raw = window.localStorage.getItem(oldKey);
    if (raw !== null && window.localStorage.getItem(newKey) === null) {
      window.localStorage.setItem(newKey, raw);
    }
    window.localStorage.removeItem(oldKey);
  };
  migrateThemeKey('banri-chenguang.pref.themeMode', 'taibu.pref.themeMode');
  migrateThemeKey('banri-chenguang.pref.theme', 'taibu.pref.theme');

  const storedThemeMode = parseStoredValue('banri-chenguang.pref.themeMode');
  const legacyTheme = parseStoredValue('banri-chenguang.pref.theme');
  const themeMode =
