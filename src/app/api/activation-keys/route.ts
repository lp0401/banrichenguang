...
import { jsonError, jsonOk, requireAdminContext, requireUserContext } from "@/lib/api-utils";
import {
...
export async function GET(request: NextRequest) {
    try {
        ...
        const keys = await getAllActivationKeys(filters, { client: auth.db });
...
export async function POST(request: NextRequest) {
    try {
        ...
        if (body.action === "create") {
            ...
            const result = await createActivationKeys(auth.user.id, params, { client: auth.db });
...
export async function DELETE(request: NextRequest) {
    try {
        ...
        const success = await deleteActivationKey(keyId, { client: auth.db });
