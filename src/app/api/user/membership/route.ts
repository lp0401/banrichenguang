...
import { ensureUserRecordWithFallback } from '@/lib/user/profile-record';
...
export async function GET(request: NextRequest) {
    ...
    const loadMembershipRow = async () => await auth.db
        .from('users')
        .select('membership, membership_expires_at, ai_chat_count, is_admin')
        .eq('id', auth.user.id)
        .maybeSingle();

    let { data, error } = await loadMembershipRow();
    if (!data && !error) {
        const ensured = await ensureUserRecordWithFallback(auth.user, auth.db);
        if (ensured.ok) {
