...
test('generateMcpKeyCode uses sk-mcp-banri-chenguang- prefix', async () => {
  const { generateMcpKeyCode } = await import('../lib/mcp-keys');
  const key = generateMcpKeyCode();

  assert.ok(key.startsWith('sk-mcp-banri-chenguang-'));
  assert.equal(key.length, 'sk-mcp-banri-chenguang-'.length + 24);
});
        ...
        data: [
          {
            id: 'key-1',
            user_id: 'user-1',
            key_code: 'sk-mcp-banri-chenguang-abcdef1234567890abcdef12',
            is_active: true,
            is_banned: false,
